...
%%==============================================================================
-module(els_dap_group_leader_server).
...
new() ->
    Caller = self(),
    GL = group_leader(),
    supervisor:start_child(els_dap_group_leader_sup, [#{caller => Caller, gl => GL}]).
...
handle_call({flush}, _From, #{acc := Acc} = State) ->
    Res = els_dap_utils:to_binary(lists:flatten(lists:reverse(Acc))),
    {reply, Res, State};
