...
-module(els_dap_distribution_server).
...
start_distribution(Name) ->
    Cookie = els_dap_config_runtime:get_cookie(),
    RemoteNode = els_dap_config_runtime:get_node_name(),
    NameType = els_dap_config_runtime:get_name_type(),
...
handle_call({connect}, _From, State) ->
    Node = els_dap_config_runtime:get_node_name(),
...
handle_call({rpc_call, M, F, A, Timeout}, _From, State) ->
    {ok, P} = els_dap_group_leader_server:new(),
    Node = els_dap_config_runtime:get_node_name(),
    ?LOG_INFO("RPC Call [node=~p] [mfa=~p]", [Node, {M, F, A}]),
    Result = rpc:call(Node, M, F, A, Timeout),
    Output = els_dap_group_leader_server:flush(P),
    ok = els_dap_group_leader_server:stop(P),
...
-spec start(atom()) -> ok.
start(Node) ->
    Cmd = els_dap_config_runtime:get_start_cmd(),
    Args = els_dap_config_runtime:get_start_args(),
    Path = els_dap_config_runtime:get_otp_path(),
    ?LOG_INFO(
        "Starting new Erlang node [node=~p] [cmd=~p] [args=~p] [path=~p]",
        [Node, Cmd, Args, Path]
    ),
    spawn_link(fun() -> els_dap_utils:cmd(Cmd, Args, Path) end),
...
node_name(Prefix, Name0) ->
    ...
    els_dap_utils:compose_node_name(Id, els_dap_config_runtime:get_name_type()).
